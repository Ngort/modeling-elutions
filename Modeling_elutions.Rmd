...
for(p in 1:P){
    for(f in 1:F){
        counts[p,f] ~ normal(pow(f-mu[p],2)/(-2*pow(sigma_in,2)),sigma_out);
    }
}
}
...
for(p in 1:P){
    for(f in 1:F){
        counts_hat[p,f] = normal_rng(pow(f-mu[p],2)/(-2*pow(sigma_in,2)),sigma_out);
    }
}
...
ggplot(gather(posteriors,key='parameter'),aes(y=parameter,x=value,fill=parameter,color=parameter)) + geom_density_ridges(scale=3,alpha=0.5) + scale_x_continuous(limits=c(min(posteriors),20))
```
